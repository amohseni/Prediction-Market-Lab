...
pm_glossary <- function() {
  ...
  do.call(rbind, list(
    ...
    g("Aggregation efficiency (AE)", "The fraction of achievable forecast improvement the market captures: \\((B_{\\text{prior}} - B_{\\text{market}})/(B_{\\text{prior}} - B_{\\text{omniscient}})\\). \\(1\\) = perfect aggregation; below \\(0\\) = worse than the prior."),
    ...
    g("Brier score", "Squared error of a probability forecast, \\((\\text{forecast} - \\text{outcome})^2\\). \\(0\\) is perfect; \\(0.25\\) is the score of always saying 50%."),
    ...
    g("Effective sample size (n_eff)", "The number of independent signals that would carry the same information as \\(n\\) correlated ones: \\(n_{\\text{eff}} = n/(1+(n-1)\\rho)\\). Tends to \\(1/\\rho\\) as \\(n\\) grows -- the information ceiling."),
    ...
    g("Kelly criterion", "Bet the bankroll fraction that maximizes expected log wealth (equivalently, long-run growth). For a binary contract: \\(f^\\star = (\\text{belief} - \\text{price})/(1 - \\text{price})\\)."),
    ...
    g("Sufficient statistic", "A summary of data that preserves all its information about the unknown -- here, the average signal \\(\\bar s\\).")
  ))
}
...
<p>A hidden number \\(\\theta\\) is the truth &mdash; say, a candidate's true vote share. Nobody sees it. The market trades a <b>binary contract</b> on a yes/no question: does \\(\\theta\\) clear a threshold \\(c\\)? The contract pays $1 if yes and $0 if no, so its fair price is just the probability of yes.</p>
<p>Everyone starts with the same <b>prior</b> (a bell curve for \\(\\theta\\)). Each of the \\(n\\) traders then gets a private <b>signal</b> &mdash; a noisy reading of the truth. The noise is <b>correlated</b>: a fraction \\(\\rho\\) of it is a <i>shared</i> mistake (a misleading poll, a media narrative) and the rest is each trader's own. Traders update by Bayes' rule to a belief about the event.</p>
<p>Two forecasts bracket the market. The <b>prior</b> \\(p_0\\) is what you'd say with no market at all; any market worth having must beat it. The <b>best possible</b> forecast \\(p^\\star\\) pools <i>all</i> signals at once. The key fact: correlated signals are worth only \\(n_{\\text{eff}} = n/(1 + (n-1)\\rho)\\) independent ones, which tends to \\(1/\\rho\\) as the crowd grows. With \\(\\rho = 0.1\\), a million traders know at most as much as ten independent ones. <b>Correlation is a hard ceiling on what any market can know.</b> The real question is never &ldquo;is the price accurate?&rdquo; but &ldquo;how much of the gap between the prior and the best possible does it close?&rdquo; &mdash; the <i>accuracy-efficiency</i> \\(\\text{AE}\\).</p>

<h4>The market maker</h4>
<p>Trades run against an <b>LMSR</b> automated market maker (Hanson's logarithmic market scoring rule) instead of a matching order book. It always quotes a price in \\((0,1)\\), and one parameter \\(b\\) (liquidity) sets how much money it takes to move that price. The maker can lose money &mdash; that subsidy is what pays traders to reveal information &mdash; but never more than \\(b\\ln 2\\).</p>

<h4>The traders</h4>
<p>Wealth is heavy-tailed (a few whales), and each trader bets a <b>Kelly</b> fraction of their wealth toward their belief &mdash; the stake that maximizes long-run growth. In the frictionless case theory pins the resting price exactly: the <b>wealth-weighted average of beliefs</b> (Beygelzimer-Langford-Pennock). Rich traders' opinions count for more. Besides <i>informed</i> traders there are <i>noise</i> traders (beliefs unrelated to the truth; they lose on average and thereby pay the informed) and <i>manipulators</i> (who ignore information and push toward a target price \\(\\pi^\\star\\)).</p>

<h4>Three frictions, three mechanisms</h4>
<p>All three cost parameters make trading costly, but they bite differently:</p>
<ul>
<li><b>Proportional fee \\(\\tau\\)</b> (intensive margin): shrinks every trade, so the price is pulled toward its start &mdash; <i>shrinkage</i>.</li>
<li><b>Fixed cost \\(\\kappa\\)</b> (extensive, per trade): kills small trades entirely &mdash; <i>censoring</i>.</li>
<li><b>Participation cost \\(c_{\\text{part}}\\)</b> (extensive, per market, decided <i>before</i> seeing prices): decides who even shows up &mdash; <i>sample selection</i>.</li>
</ul>
<p>All three open a <b>no-trade band</b>: a gap between belief and price too small to be worth trading, which leaves the price <b>stale</b>. Only \\(c_{\\text{part}}\\) can cleanly produce the Hanson-Oprea effect (manipulation drawing dormant traders in) &mdash; though in this model, as the Reliability and validation results show, that benefit is elusive.</p>

<h4>Herding</h4>
<p>With herding (weight \\(h\\)), traders partly <b>adopt the price as their own belief</b>. This rewrites the beliefs that feed the price, so a distortion held for a few rounds can <b>outlive</b> the trader who caused it &mdash; a genuine information cascade. With \\(h = 0\\) the market is unreasonably well-behaved: manipulation is self-defeating and fees only slow things down. Turn \\(h\\) up to see cascades in Run Anatomy.</p>
  ...
  moduleServer(id, function(input, output, session) {
    output$glossary <- DT::renderDataTable({
      DT::datatable(
        ...
        rownames = FALSE,
        options = list(
          pageLength = 15, dom = "ft", autoWidth = FALSE,
          columnDefs = list(list(width = "220px", targets = 0)),
          # DT renders (and re-renders on search/page) client-side; re-typeset the
          # LaTeX in the definitions with MathJax after every draw.
          drawCallback = DT::JS(
            "function() { if (window.MathJax && MathJax.Hub)",
            "  MathJax.Hub.Queue(['Typeset', MathJax.Hub, this.api().table().node()]); }")),
